...
class Event extends React.Component {
	...
	eventListVisible()
	{
		...
		let b = false
		let c = false
		for (const event in this.state.EventList)
		{
			if (this.state.EventList[event].status == "ongoing") {
				a = true
			}
			else if (this.state.EventList[event].status == "pending") {
				b = true
			}
			else if (this.state.EventList[event].status == "finished") {
				c = true
			}
		}
		
		this.setState({ongoingVisible: a})
		this.setState({pendingVisible: b})
		this.setState({finishedVisible: c})
	}
	
		async fetchEventList(search)
		{
			await axios.post(
	...
	render()
	{
		return(
			<View style={{flex: 1}}>
				...
				<EventSearchContext.Provider value={this.state.searchQuery}>
				<Searchbar
					...
					onChangeText={(text) => {
						this.fetchEventList(text);
					}}
				/>
	
				<ScrollView>
					{
						this.state.ongoingVisible == true?
						<View>
							{
								this.state.ongoingVisible == true?
					...
					{
						this.state.finishedVisible == true?
						<View>
					...
					{
						this.state.pendingVisible == true?
						<View>
